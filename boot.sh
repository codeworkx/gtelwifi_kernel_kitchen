...
if [ -f ${CUR_DIR}/${TEMP_DIR}/kernel/arch/arm/boot/zImage ] &&  [ -f ${TEMP_DIR}/ramdisk.cpio.gz ] && [ -f ${TEMP_DIR}/dt.img ]; then
    ...
    mkbootimg --kernel ${CUR_DIR}/${TEMP_DIR}/kernel/arch/arm/boot/zImage --ramdisk ${TEMP_DIR}/ramdisk.cpio.gz --dt ${TEMP_DIR}/dt.img --cmdline 'console=ttyS1,115200n8' --base 0x00008000 --ramdisk_offset 0x01000000 --second_offset 0x00f00000 --tags_offset 0x00000100 --board 'sc8830' --pagesize 2048 -o ${OUT_DIR}/boot.img
else
    echo "Can't create boot.img because of missing input files"
fi
